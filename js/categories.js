...
function displayData(){
    ...
    for(let i=0 ; i<result.length ; i++ ){
        index.push(`${result[i].strCategory}`);
        cartona+=`
        <div class="col-md-3 ">
                <div class="image-div"  onclick="getMealsCat(${i})" >
...
function getMealsCat(hamada){
    let currentNameCate = index[hamada];
    
    async function getMeals(){
        const api =await fetch(`https://www.themealdb.com/api/json/v1/1/filter.php?c=${currentNameCate}`);
