...
function displayData(){
    ...
    for(let i=0 ; i<result.length ; i++ ){
        areaArr.push(`${result[i].strArea}`);
        cartona+=`
            <div class="col-md-3">
                <div onclick = "getAreaMeals(${i})">
...
function getAreaMeals(index){
    let currentArea = areaArr[index];

    async function getMeals(){
        const api =await fetch(`https://www.themealdb.com/api/json/v1/1/filter.php?a=${currentArea}`);
    ...
    function displayData(){
        ...
        document.getElementById('inner-area').classList.add('d-none');
